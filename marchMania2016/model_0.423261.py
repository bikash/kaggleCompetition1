...
for index, row in train.iterrows():
    ...
    e = elo.rate_1vs1(team[t1], team[t2])
...
for index, row in preds.iterrows():
    ...
    prediction[i] = 0.5 + (team[p[1]] - team[p[2]]) / 400.0
    i += 1

preds['Pred'] = np.clip(prediction, 0.01, 0.99)
preds.to_csv('output/Prediction.csv', index=False)
