...
for (i in 1:nrow(ds1.clean)) {
  temp <- ngramMatches(ds1.clean$Title[i], ds1.clean$Question[i])
  X2 <- rbind(X2, temp)
}

# rename columns
names(X2) <- c( "T.len", "D.len", "PT.1grms", "PT.2grms", "PT.3grms", "PT.4grms",
...
for (i in 1:nrow(ds2.clean)) {
  temp <- ngramMatches(ds2.clean$Title[i], ds2.clean$Question[i])
  X2.test <- rbind(X2.test, temp)
}

# Rename columns
names(X2.test) <- c("PT.len", "PD.len", "PT.1grms", "PT.2grms", "PT.3grms", "PT.4grms",
                    "PD.1grms", "PD.2grms", "PD.3grms", "PD.4grms", "PT.ngrm.matchscore", "PD.ngrm.matchscore",
                    "PT.ngrm.simscore", "PD.ngrm.simscore", "PT.2grms.2ndorder", "PD.2grms.2ndorder",
                    "PT.similarity", "PD.similarity")
