...
def item2str(query_str):
    query_str = query_str.replace('__________', 'what')
    query_str = query_str.replace('/', ' ')
    query_str = query_str.replace('\\', ' ')
    query_str = query_str.replace(')', '')
    query_str = query_str.replace('(', '')
    query_list = [ str.lower().strip('?').strip('.').strip(',').strip('!').strip('(').strip(')').strip(':') for str in query_str.split()]
...
def get_save_wiki_per_doc(keywords, save_folder = 'data/wiki_data/'):
    ...
    try:
        content = wiki.page(kw).content.encode('ascii', 'ignore')
        fname = os.path.join(save_folder, '_'.join(kw.split()) + '.txt')
        if not os.path.exists(fname):
            with open(os.path.join(save_folder, '_'.join(kw.split()) + '.txt'), 'w') as f:
                f.write(content)
                print "page %s size %d" %(kw, len(content))
        count = count + 1
        if count % 100 == 0:
            print count
    except wiki.exceptions.DisambiguationError as e:
...
def get_wiki_docs(wiki_pages='./data/keywords.txt'):
    with open(wiki_pages) as f:
        pages = f.readlines()
    pages = [item.strip()  for item in pages]
    for page in pages:
        get_save_wiki_per_doc(page)
        #dr = Parallel(n_jobs=36)(delayed(get_save_wiki_per_doc)(page) for page in pages)
    return 
...
def get_docstf_idf(dir_data):
    ...
    print >> sys.stderr,  "Finish calculate doc tf idf"
    return docs_tf, idf
